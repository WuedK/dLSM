...
static char conn_config_file_name[100] = "../connection_bigdata.conf";
...
//  uint64_t deallocation_buffers[REMOTE_DEALLOC_BUFF_SIZE / sizeof(uint64_t)];
  ...
  std::map<uint8_t, std::atomic<int>> compute_nodes_status{};   // if status is less than 0 there is a problem with the connection
